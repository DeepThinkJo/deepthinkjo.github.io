...
We say $\lim_{x \to a} f(x) = L$ if for every $\epsilon > 0$, there exists a $\delta > 0$ such that:
...
* **Note**: The converse is **not** necessarily true. A function can be continuous but not differentiable (e.g., $f(x) = |x|$ at $x=0$).
