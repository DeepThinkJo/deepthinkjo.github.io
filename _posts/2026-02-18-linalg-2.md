---
title: "Linear Algebra (2): Linear Transformations and Matrices"
date: 2026-02-18
...
For a linear transformation $T: V \to W$ where $V$ and $W$ are finite-dimensional vector spaces with ordered bases $\beta$ and $\gamma$, respectively, the **matrix representation** of $T$, denoted $[T]_\beta^\gamma$ is the unique matrix such that for every vector $\mathbf{v} \in V$:

$$
[T(\mathbf{v})]_\gamma = [T]_\beta^\gamma [\mathbf{v}]_\beta.
$$
...
A linear transformation $T: V \to W$ is an **Isomorphism** if it is bijective. If such a mapping exists, $V$ and $W$ are **isomorphic**.
