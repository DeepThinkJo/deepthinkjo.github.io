...
* **$L_1$ Norm (Manhattan):** $\lVert\mathbf{x}\rVert_{1}=\sum\lvert x_{i}\rvert$
* **$L_2$ Norm (Euclidean):** $\lVert\mathbf{x}\rVert_{2}=\sqrt{\sum x_{i}^{2}}$
* **$L_\infty$ Norm (Maximum):** $\lVert\mathbf{x}\rVert_{\infty}=\max\lvert x_{i}\rvert$
